<script lang="ts">
    ...
    import { deleteBoardTarget, setDeleteBoardTarget } from "$lib/stores/board";
    import { deleteBoard } from "$lib/api/boardApi";

    let confirmText = "";
    let isDeleting = false;

    $: boardName = $deleteBoardTarget?.name || "";
    $: canDelete = confirmText.trim() === boardName && !isDeleting;
    ...
    async function handleDelete() {
        if (!$deleteBoardTarget || !canDelete) return;
...
{#if $activeModal === "deleteBoardModal" && $deleteBoardTarget}
            ...
            >
                <div class="p-6">
                    <div class="flex items-center gap-3 mb-4 text-red-500">
                        ...
                        <h3 class="text-lg font-bold text-[#111418] dark:text-white">Delete Board</h3>
                    </div>

                    <p class="text-sm text-[#5c6b7f] dark:text-gray-400 mb-4">
                        This will permanently delete
                        <span class="font-semibold text-[#111418] dark:text-white">{$deleteBoardTarget.name}</span>
                        and all its lists and tasks.
                    </p>
                    ...
                    <div class="mb-5">
                        ...
                        <input
                            ...
                            placeholder={$deleteBoardTarget.name}
                            autofocus
                        />
                    </div>
